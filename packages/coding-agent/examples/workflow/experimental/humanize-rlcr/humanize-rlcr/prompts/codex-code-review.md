...
Classify diff hygiene problems as blocking when they affect the current work:
broad formatter/style/import/order churn, unrelated cleanup, generated artifacts
mixed into project changes, untracked claimed files without rationale, or changed
files outside the task's acceptance surface. Require the fix round to revert
unrelated churn instead of normalizing the large diff after the fact.
