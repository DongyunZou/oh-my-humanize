...
export class PythonKernel extends BaseKernel {
	private constructor(id: string) {
		super(id, {
			...
			signalProcess: signalPythonKernelProcessTree,
	...
	static async start(options: KernelStartOptions): Promise<PythonKernel> {
		...
		const proc = Bun.spawn([runtime.pythonPath, "-u", scriptPath], {
			...
			// commands. On POSIX, isolate the runner into its own process group so
			// cancellation can stop the whole descendant tree without signalling the
			// interactive OMH host.
