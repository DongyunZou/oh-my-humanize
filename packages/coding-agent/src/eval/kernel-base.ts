...
export interface BaseKernelOptions<TExecuteOptions extends KernelExecuteOptions = KernelExecuteOptions> {
	...
	/** Sends a lifecycle signal to the runner process. Defaults to Bun's direct subprocess signal. */
	signalProcess?: (proc: Subprocess, signal: NodeJS.Signals) => void;
}
...
export abstract class BaseKernel<TExecuteOptions extends KernelExecuteOptions = KernelExecuteOptions> {
	...
	async interrupt(): Promise<void> {
		...
		try {
			this.#signalProcess(this.#proc, "SIGINT");
		} catch (err) {
	...
	#signalProcess(proc: Subprocess, signal: NodeJS.Signals): void {
		if (this.#options.signalProcess) {
			this.#options.signalProcess(proc, signal);
			return;
		}
		proc.kill(signal);
	}
