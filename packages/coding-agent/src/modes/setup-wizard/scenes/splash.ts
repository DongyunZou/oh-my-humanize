...
import { gradientEscape, gradientLogo, OMH_LOGO, type ShineConfig } from "../../components/welcome";
...
const LARGE_LOGO = OMH_LOGO.flatMap(line => {
	let wide = "";
...
/**
 * Animated setup splash, in the spirit of the OMH landing page: the brand
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	const art = height >= 14 ? LARGE_LOGO : OMH_LOGO;
	const content = [...gradientLogo(art, phase, shine), "", theme.bold("O h   M y   H u m a n i z e")];
