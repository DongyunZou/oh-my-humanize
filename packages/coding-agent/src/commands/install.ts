/**
 * `omh install <target>` — top-level convenience over `omh plugin install` /
 * `omh plugin link`.
 *
 * The docs (omh.sh/docs/extension-authoring) advertise
 *
 *   omh install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `omh install <target>` should `link` a
