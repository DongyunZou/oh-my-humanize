...
The compiled macOS `omh` binaries shipped on GitHub Releases are signed with a
...
submission (see [#776](https://github.com/humanfia/oh-my-humanize/issues/776)).
...
- `curl https://omh.sh/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/omh-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/omh-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/omh-signing             # upload all five
gh secret list --repo humanfia/oh-my-humanize                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/omh-darwin-arm64
```
