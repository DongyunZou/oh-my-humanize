#!/usr/bin/env bun
//
// Render the Homebrew formula for `omh` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (omh-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/omh.rb>
//   bun scripts/ci-update-brew-formula.ts v15.10.3        # prints to stdout

import { $ } from "bun";

const REPO = process.env.OMP_REPO ?? "humanfia/oh-my-humanize";
const HOMEPAGE = "https://omh.sh";
const DESC = "Human-guided workflow coding agent";
...
// returns `nil` and `bin.install nil => "omh"` raises.
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/omh-darwin-arm64",
          using: :nounzip
      sha256 "${sums["omh-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/omh-darwin-x64",
          using: :nounzip
      sha256 "${sums["omh-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/omh-linux-arm64",
          using: :nounzip
      sha256 "${sums["omh-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/omh-linux-x64",
          using: :nounzip
      sha256 "${sums["omh-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["omh-*"].first => "omh"
    (bin/"omh").chmod 0555
    bin.install_symlink "omh" => "omp"
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"omh", "completions", shells: [:bash, :zsh, :fish])
    end
  end
...
async function main(): Promise<void> {
	...
	const targets = ["omh-darwin-arm64", "omh-darwin-x64", "omh-linux-arm64", "omh-linux-x64"];
