...
install_binary() {
    ...
    curl -fsSL "$BINARY_URL" -o "${INSTALL_DIR}/omh"
    chmod +x "${INSTALL_DIR}/omh"
    ln -sfn "omh" "${INSTALL_DIR}/omp"
    echo ""
    echo "✓ Installed omh to ${INSTALL_DIR}/omh"

    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'omh' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'omh'" ;;
    esac
}
